...
public class TimerController {
		...
		currentTimer = new CountDownTimer(RECORDING_LIMIT, 1) {
			...
			public void onFinish() {
				try {
					...
					TEXTVIEWS.get("notify").startAnimation(FadeOut);
					TEXTVIEWS.get("notify").setVisibility(View.INVISIBLE);
	...
	public void animateNotifyStart() {
		// Animate "Please say your child's name" to visible
		TEXTVIEWS.get("notify").setVisibility(View.VISIBLE);
		...
		Animation SlideIn = AnimationUtils.loadAnimation(context, R.anim.slide_left);
			SlideIn.setAnimationListener(new AnimationListener() {
				...
				public void onAnimationEnd(Animation animation) {					
					TEXTVIEWS.get("notify").startAnimation(SlideDown);
				}
			});
			SlideIn.setFillAfter(true);
		TEXTVIEWS.get("notify").startAnimation(SlideIn);		
	}
